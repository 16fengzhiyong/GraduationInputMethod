...
class NiceImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : AppCompatImageView(context, attrs, defStyleAttr) {
    ...
    private fun drawBorders(canvas: Canvas) {
        if (isCircle) {
            ...
            if (innerBorderWidth > 0) {
                drawCircleBorder(canvas, innerBorderWidth, innerBorderColor, radius - borderWidth.toFloat() - innerBorderWidth / 2.0f)
            }
        } else {
            if (borderWidth > 0) {
    ...
    private fun initSrcRectF() {
        if (isCircle) {
            ...
            srcRectF!!.set(0f, 0f, width.toFloat(), height.toFloat())
            if (isCoverSrc) {
    ...
    fun setBorderWidth(borderWidth: Int) {
        this.borderWidth = Utils.dp2px(context, borderWidth.toFloat())
    ...
    fun setInnerBorderWidth(innerBorderWidth: Int) {
        this.innerBorderWidth = Utils.dp2px(context, innerBorderWidth.toFloat())
        clearInnerBorderWidth()
        invalidate()
    }
    ...
    fun setCornerRadius(cornerRadius: Int) {
        this.cornerRadius = Utils.dp2px(context, cornerRadius.toFloat())
        calculateRadiiAndRectF(false)
    }

    fun setCornerTopLeftRadius(cornerTopLeftRadius: Int) {
        this.cornerTopLeftRadius = Utils.dp2px(context, cornerTopLeftRadius.toFloat())
        calculateRadiiAndRectF(true)
    }

    fun setCornerTopRightRadius(cornerTopRightRadius: Int) {
        this.cornerTopRightRadius = Utils.dp2px(context, cornerTopRightRadius.toFloat())
        calculateRadiiAndRectF(true)
    }

    fun setCornerBottomLeftRadius(cornerBottomLeftRadius: Int) {
        this.cornerBottomLeftRadius = Utils.dp2px(context, cornerBottomLeftRadius.toFloat())
        calculateRadiiAndRectF(true)
    }

    fun setCornerBottomRightRadius(cornerBottomRightRadius: Int) {
        this.cornerBottomRightRadius = Utils.dp2px(context, cornerBottomRightRadius.toFloat())
        calculateRadiiAndRectF(true)
    }
